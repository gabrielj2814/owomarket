...
import { Breadcrumb, BreadcrumbItem, Button, Card, HelperText, Label, Select, Spinner, Textarea, TextInput, ToggleSwitch } from 'flowbite-react';
import { FC, useEffect, useState } from 'react';
import { HiHome, HiPlus, HiTrash } from 'react-icons/hi';
import { LuArrowBigLeft, LuBoxes, LuImage, LuLayers, LuPackage, LuRuler, LuSave, LuSaveOff, LuTag } from 'react-icons/lu';
...
const FormProductPage: FC<FormProductPageProps> = ({ user_id, title, host, user_name, record_uuid }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.code === 200 || res.code === 201 || res.status === 'success' || res.code === 200 || res.code === 201) {
        ...
        <>
            ...
            <Dashboard user_uuid={user_id}>
                ...
                >
                    ...
                    <BreadcrumbItem href={`/product/backoffice/${user_id}/module`}>Productos</BreadcrumbItem>
                ...
                <div className="mb-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
                    ...
                    <Button color="gray" onClick={regresar} className="shadow-sm">
                        <LuArrowBigLeft className="mr-1 h-5 w-5" />
                        Regresar
                    </Button>
                </div>
                ...
                {loadingData ? (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <Card className="border-gray-100 shadow-sm dark:border-gray-700">
                            <h2 className="mb-3 flex items-center gap-2 text-lg font-bold text-gray-900 dark:text-white">
                                <LuTag className="h-5 w-5 text-blue-500" />
                        ...
                        <Card className="border-gray-100 shadow-sm dark:border-gray-700">
                            <h2 className="mb-3 flex items-center gap-2 text-lg font-bold text-gray-900 dark:text-white">
                                <LuLayers className="h-5 w-5 text-purple-500" />
                            ...
                            <div className="flex flex-wrap gap-6 border-t border-gray-100 pt-2 dark:border-gray-700">
                                <ToggleSwitch
                        ...
                        <Card className="border-gray-100 shadow-sm dark:border-gray-700">
                            <h2 className="mb-3 flex items-center gap-2 text-lg font-bold text-gray-900 dark:text-white">
                                <LuBoxes className="h-5 w-5 text-emerald-500" />
                                3. Inventario y Descripciones
                            </h2>
                        ...
                        <Card className="border-gray-100 shadow-sm dark:border-gray-700">
                            <div className="mb-1 flex items-center justify-between">
                                <h2 className="flex items-center gap-2 text-lg font-bold text-gray-900 dark:text-white">
                                    <LuImage className="h-5 w-5 text-cyan-500" />
                                    4. Galería de Imágenes
                                </h2>
                                <span className="text-xs text-gray-500">{(form.images || []).length} / 8 imágenes cargadas</span>
                            </div>
                            <p className="mb-3 text-xs text-gray-500 dark:text-gray-400">
                                Sube fotos directamente al almacenamiento de tu tienda o arrastra los archivos. La imagen con la etiqueta "Portada" se
                                mostrará como imagen principal en el catálogo.
                            </p>
                        ...
                        <Card className="border-gray-100 shadow-sm dark:border-gray-700">
                            <div className="mb-3 flex items-center justify-between">
                                <h2 className="flex items-center gap-2 text-lg font-bold text-gray-900 dark:text-white">
                                    <LuLayers className="h-5 w-5 text-indigo-500" />
                            ...
                            {form.variants && form.variants.length > 0 ? (
                                <div className="space-y-3">
                                    {form.variants.map((v, i) => (
                                        <div
                                            key={i}
                                            className="grid grid-cols-1 items-end gap-3 rounded-lg border border-gray-100 bg-gray-50/50 p-3 sm:grid-cols-4 dark:border-gray-700 dark:bg-gray-800"
                                        >
                                            <div>
                                            ...
                                            <div>
                                                ...
                                                <TextInput
                                                    ...
                                                    min={0}
                                                    value={v.quantity}
                                                    onChange={(e) => handleVariantChange(i, 'quantity', parseInt(e.target.value, 10) || 0)}
                                                    required
                                                />
                                            </div>
                                            <div className="flex justify-end">
                                                <Button size="xs" color="failure" onClick={() => handleRemoveVariant(i)} type="button">
                                ...
                                <div className="rounded-xl border-2 border-dashed border-gray-200 py-6 text-center text-sm text-gray-400 dark:border-gray-700">
                        ...
                        <div className="flex flex-col justify-end gap-3 pt-4 sm:flex-row">
                            <Button color="gray" onClick={regresar} disabled={saving}>
                                <LuSaveOff className="mr-1 h-5 w-5" />
                                Cancelar
                            </Button>
                            <Button color="blue" type="submit" disabled={saving} className="shadow-md">
                                {saving ? <Spinner size="sm" className="mr-2" /> : <LuSave className="mr-1 h-5 w-5" />}
