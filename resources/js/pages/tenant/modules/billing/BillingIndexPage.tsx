...
const BillingIndexPage: FC<BillingIndexPageProps> = ({ user_id, title }) => {
    ...
    const handleCreateInvoice = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res?.data?.data) {
                showToast(`Factura ${res.data.data.invoice_number} emitida exitosamente`);
                setCreateModalOpen(false);
                setForm(initialInvoiceForm);
                fetchMetrics();
                fetchInvoices(1);
            } else if (res?.data?.errors) {
                setFormErrors(res.data.errors as Record<string, string[]>);
            } else {
                showToast(res?.data?.message || "Ocurrió un error al emitir la factura", "error");
            }
        } catch (error: any) {
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Link href={`/billing/backoffice/${user_id}/settings`}>
                            <Button color="light">
                                <LuReceiptText className="w-4 h-4 mr-2 text-gray-600" />
                                Datos Fiscales
                            </Button>
                        </Link>
                    </div>
                    ...
                    <Card className="border-l-4 border-l-green-600">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="p-3 bg-green-50 dark:bg-green-900/30 rounded-xl">
                                <LuFileCheck className="w-6 h-6 text-green-600 dark:text-green-400" />
                            </div>
                        </div>
                    </Card>
                    ...
                    <Card className="border-l-4 border-l-cyan-600">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="p-3 bg-cyan-50 dark:bg-cyan-900/30 rounded-xl">
                                <HiDocumentText className="w-6 h-6 text-cyan-600 dark:text-cyan-400" />
                            </div>
                        </div>
                    </Card>
                                ...
                                <TableBody className="divide-y">
                                    {invoices.map((inv) => (
                                        <TableRow key={inv.id} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                                            <TableCell className="font-bold text-blue-600 dark:text-blue-400 whitespace-nowrap">
                                            ...
                                            <TableCell>
                                                <div className="font-medium text-gray-900 dark:text-white">
                                                    {inv.billing_customer_name}
                                                </div>
                                                <div className="text-xs text-gray-500">
                                                    {inv.billing_customer_email} {inv.billing_customer_tax_id ? `• ${inv.billing_customer_tax_id}` : ""}
                                                </div>
                                            </TableCell>
                                            ...
                                            <TableCell className="text-right whitespace-nowrap">
                                                <div className="flex items-center justify-end gap-1">
                                                    ...
                                                    >
                                                        <HiDocumentDownload className="w-3.5 h-3.5 text-blue-600" />
                                                    </Button>
                                                    ...
                                                    >
                                                        <HiMail className="w-3.5 h-3.5 text-gray-600" />
                                                    </Button>
                ...
                <form onSubmit={handleCreateInvoice}>
                    <ModalBody className="space-y-4 max-h-[75vh] overflow-y-auto">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                <div>
                                    ...
                                    {formErrors.customer_name && (
                                        <span className="text-xs text-red-600">{formErrors.customer_name[0]}</span>
                                    )}
                                </div>
                                ...
                                <div>
                                    ...
                                    {formErrors.customer_email && (
                                        <span className="text-xs text-red-600">{formErrors.customer_email[0]}</span>
                                    )}
                                </div>
                        ...
                        <div className="border-b pb-3 dark:border-gray-700">
                            <div className="flex items-center justify-between mb-2">
                                <h4 className="text-sm font-semibold text-gray-900 dark:text-white uppercase">
                                    2. Conceptos e Ítems a Facturar
                                </h4>
                            ...
                            <div className="space-y-3">
                                {form.items.map((item, idx) => (
                                    <div key={idx} className="p-3 bg-gray-50 dark:bg-gray-700/40 rounded-lg flex flex-col md:flex-row gap-2 items-start md:items-center">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg space-y-2 text-sm">
                                ...
                                <div className="flex justify-between text-lg font-bold text-blue-600 border-t pt-2 dark:border-gray-600">
                                    <span>TOTAL:</span>
                                    <span>${calculatedTotal.toFixed(2)} {form.currency}</span>
                                </div>
                            </div>
                        </div>
                    </ModalBody>
                    <ModalFooter>
                        <Button color="blue" type="submit" disabled={submitting}>
                            {submitting ? <Spinner size="sm" className="mr-2" /> : <HiPlus className="w-4 h-4 mr-2" />}
                            Emitir Factura
                        </Button>
            ...
            <Modal show={cancelModalOpen} onClose={() => setCancelModalOpen(false)} size="md">
                ...
                <ModalFooter>
                    <Button color="failure" onClick={handleCancelInvoice} disabled={cancelling}>
                        {cancelling ? <Spinner size="sm" className="mr-2" /> : <HiTrash className="w-4 h-4 mr-2" />}
                        Confirmar Anulación
                    </Button>
            ...
            <Modal show={resendModalOpen} onClose={() => setResendModalOpen(false)} size="md">
                ...
                <ModalFooter>
                    <Button color="blue" onClick={handleResendMail} disabled={resending}>
                        {resending ? <Spinner size="sm" className="mr-2" /> : <HiMail className="w-4 h-4 mr-2" />}
                        Enviar Correo
                    </Button>
