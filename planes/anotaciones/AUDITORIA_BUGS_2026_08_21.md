...
**Estado al 22/08/2026 (tarde): 35 cerrados · 1 abierto.** Repasados uno a uno contra el código,
...
| N17 | Los despachos fallidos quedan en `status = 'failed'` y nada los reintenta; el despacho sigue siendo síncrono | Fase 1.1 | ✅ Cerrado (22/08) — `DispatchCentralOrderJob` con 5 reintentos y espera creciente. **Antes hubo que arreglar `reserveDispatch()`**, que devolvía null en cuanto existía fila: una tienda en `failed` se saltaba en todos los reintentos, así que encolar sin más no habría reintentado nada. Columna `attempts` con tope de 5 |
| N18 | 🟡 **Parcial.** La Fase 4.1 puso `throttle:5,15` en las dos rutas del PIN, pero **el resto de la aplicación sigue sin ningún límite de tasa**: login, registro y consumo de SSO son los siguientes candidatos. Texto original: ningún endpoint tiene límite de tasa. Al pasar `api-tenant` de `api` a `web` se perdió incluso la posibilidad de `throttleApi()` (aunque nunca estuvo activo) | Fase 0.3-E | ✅ Cerrado (22/08) — limitadores con nombre en `RouteServiceProvider`: credenciales 5/min por cuenta más 20/min por IP, altas 3/hora, SSO 10/min, y techo de 120/min sobre `/api/*` y `/api-tenant/*` |
| N19 | Dentro de una tienda no hay control de rol: un `staff` puede borrar el catálogo o anular facturas igual que el `owner` | Fase 0.3-E | ✅ Cerrado (22/08) — alias `tenant_can` con permisos Spatie por tienda y comando `tenant:provision-roles`. **Ojo: esta API lista con POST**, así que la excepción de lectura no puede ir sólo por verbo; `filter` y `calculate` se exceptúan por nombre |
| N20 | El `error` que registra el fallback prolongado del BCV **no llega a nadie**: no hay notificación ni integración con un servicio de alertas, sólo un nivel de log más alto | Fase 1.4 | ✅ Cerrado (22/08) — **decisión: correo a los superadministradores.** Contrato `StaleRateAlerter` en el dominio, canal en infraestructura. Máximo un aviso al día |
...
| N25 | La sincronización con el catálogo central es **síncrona**: escribe en la base central dentro de la misma petición, incluida la transacción del checkout. Si el marketplace no responde, la fila queda desincronizada y sólo queda el log. Lo natural es un job en cola con reintentos | Fase 2.2 | ✅ Cerrado (22/08) — `ProductObserver` encola en vez de sincronizar. El `tenant_id` se captura en el observador, donde aún hay contexto de tienda |
...
| N37 | **La API tiene TRES sobres de paginación distintos** (`data:[]`+`pagination` en raíz; `data:{data,pagination}`; `data:{data,total,per_page,...}`), y cada página del backoffice está escrita contra el suyo. Comprobado contra los endpoints reales. Unificarlos cambia respuestas HTTP en producción, así que se dejó como decisión aparte; los tres están tipados con nombre en `ResponseApi.d.ts` | Fase N29 (22/08) | ⬜ Abierto — decisión pendiente |
