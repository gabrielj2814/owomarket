...
final class EditProductUseCase
{
    ...
    public function execute(
        ...
        $existingBySlug = $this->repository->findBySlug($productSlug);
        if ($existingBySlug !== null && ! $existingBySlug->id()?->equals($productId)) {
            throw new ProductSlugAlreadyExistsException($productSlug->value());
        }

        $existingBySku = $this->repository->findBySku($productSku);
        if ($existingBySku !== null && ! $existingBySku->id()?->equals($productId)) {
